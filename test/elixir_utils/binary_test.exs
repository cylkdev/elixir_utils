defmodule ElixirUtils.BinaryTest do
  ...
  describe "chunk_bytes_stream/2 (enum)" do
    test "binary - exact multiples" do
      assert ["ab", "cd", "ef"] ==
               "abcdef"
               |> ElixirUtils.Binary.chunk_bytes_stream(2)
               |> Enum.to_list()
    end
    ...
    test "binary - emits final short chunk once" do
      assert ["abc", "d"] ==
               "abcd"
               |> ElixirUtils.Binary.chunk_bytes_stream(3)
               |> Enum.to_list()
    end
    ...
    test "binary - empty input yields empty stream" do
      assert [] ==
               ""
               |> ElixirUtils.Binary.chunk_bytes_stream(4)
               |> Enum.to_list()
    end
    ...
    test "binary - size 1 splits into single bytes" do
      assert ["a", "b", "c", "d"] ==
               "abcd"
               |> ElixirUtils.Binary.chunk_bytes_stream(1)
               |> Enum.to_list()
    end
    ...
    test "enum - empty enum yields empty stream" do
      assert [] = [] |> ElixirUtils.Binary.chunk_bytes_stream(4) |> Enum.to_list()
    end
  end
